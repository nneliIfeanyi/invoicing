...
$time = $result['t_time'];
$initial_doc = $result['initial_doc'];
$sum = 0;
...
$save = APPROOT . '/views/output/';
// ---------------------------------------------------------

//Close and output PDF document

$pdf->Output($save.'invoice'.$ran.'.pdf', 'F');
$path = 'invoice'.$ran.'.pdf';
if (empty($initial_doc)) {
  $sql = "UPDATE customers SET initial_doc = '$path' WHERE t_id = '$t_id' ";
  $query = mysqli_query($conn, $sql);
  if ($query) {
    flash('msg', 'Document generated successfully..');
    redirect('posts/index');
  }else{
    die('Something went wrong');
  }
}else{
  $sql = "UPDATE customers SET edited_doc = '$path' WHERE t_id = '$t_id' ";
  $query = mysqli_query($conn, $sql);
  if ($query) {
    flash('msg', 'Edited document generated successfully..');
    redirect('posts/index');
  }else{
    die('Something went wrong');
  }
